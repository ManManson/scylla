...
void raft_services::init_rpc_verbs() {
    ...
    auto raft_add_server_impl = [this, cas_or_bounce] (const rpc::client_info& cinfo,
        ...
        return container().invoke_on(shard_for_group(gid), [gid, addr, cas_or_bounce] (
                raft_services& self) -> future<raft::success_or_bounce> {
...
raft_services::guess_leader(raft::group_id gid) {
    raft::server& server = get_server(gid);
...
// XXX handle shutdown
future<> raft_services::rpc_until_success(raft::server_address addr, auto rpc) {
    while (true) {
...
future<> raft_services::join_raft_group0() {
...
future<> raft_services::leave_raft_group0() {
    assert(this_shard_id() == 0);
...
future<raft::peer_exchange> raft_services::peer_exchange(raft::peer_list peers) {
