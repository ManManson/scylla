...
BOOST_AUTO_TEST_CASE(test_leader_transfer_timeout) {
    ...
    // B is non-voter because we don't want it to accidentally become leader during
    // `transfer_leadership()` call.
    ...
    fd.mark_dead(C_id);
    // Start leadership transfer to C (`timeout_now` message wouldn't be delivered to C).
