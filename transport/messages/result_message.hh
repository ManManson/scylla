...
class result_message::bounce_to_shard : public result_message {
    unsigned _shard;
    std::unordered_map<uint8_t, bytes_opt> _cached_fn_calls;
public:
    bounce_to_shard(unsigned shard, std::unordered_map<uint8_t, bytes_opt> cached_fn_calls)
    ...
    std::unordered_map<uint8_t, bytes_opt>&& cached_function_calls() {
