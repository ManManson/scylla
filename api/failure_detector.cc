...
void set_failure_detector(http_context& ctx, httpd::routes& r) {
    fd::get_all_endpoint_states.set(r, [](std::unique_ptr<httpd::request> req) {
    ...
    fd::get_up_endpoint_count.set(r, [](std::unique_ptr<httpd::request> req) {
        return gms::get_up_endpoint_count().then([](int res) {
            return make_ready_future<json::json_return_type>(res);
        });
    });

    fd::get_down_endpoint_count.set(r, [](std::unique_ptr<httpd::request> req) {
        return gms::get_down_endpoint_count().then([](int res) {
            return make_ready_future<json::json_return_type>(res);
        });
    });

    fd::get_phi_convict_threshold.set(r, [] (std::unique_ptr<httpd::request> req) {
        return gms::get_phi_convict_threshold().then([](double res) {
            return make_ready_future<json::json_return_type>(res);
        });
    });

    fd::get_simple_states.set(r, [] (std::unique_ptr<httpd::request> req) {
    ...
    fd::set_phi_convict_threshold.set(r, [](std::unique_ptr<httpd::request> req) {
    ...
    fd::get_endpoint_state.set(r, [](std::unique_ptr<httpd::request> req) {
    ...
    fd::get_endpoint_phi_values.set(r, [](std::unique_ptr<httpd::request> req) {
